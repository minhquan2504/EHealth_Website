...
export function unwrapStaffList(res: any): StaffMember[] {
    ...
    return raw.map((d: any): StaffMember => ({
        id: String(d.users_id ?? d.id ?? d.staff_id ?? ''),
        code: d.code ?? d.staff_code ?? d.employee_code ?? String(d.users_id ?? ''),
        fullName: d.full_name ?? d.fullName ?? d.name ?? '',
        email: d.email ?? '',
        phone: d.phone ?? d.phone_number ?? '',
        role: Array.isArray(d.roles) && d.roles.length > 0 ? d.roles[0] : (d.role ?? 'DOCTOR'),
        roleName: d.roleName ?? d.role_name ?? '',
        departmentId: d.department_id ?? d.departmentId ?? d.department?.id ?? '',
        departmentName: d.department_name ?? d.departmentName ?? d.department?.name ?? d.specialty_name ?? '',
        specialization: d.specialty_name ?? d.specialization ?? d.specialty ?? '',
        qualification: d.qualification ?? d.degree ?? d.doctor_title ?? '',
        gender: d.gender ?? '',
        dateOfBirth: d.dob ?? d.dateOfBirth ?? d.date_of_birth ?? '',
        licenseNumber: d.licenseNumber ?? d.license_number ?? '',
        bio: d.bio ?? d.biography ?? d.description ?? '',
